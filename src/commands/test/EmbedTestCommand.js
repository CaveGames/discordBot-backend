...
module.exports = class EmbedTestCommand extends BaseCommand {
  constructor() {
  ...
  async run(client, message, args) {
	...
	const { avatar, banner, accent_color } = await axios.get(`https://discord.com/api/users/${user.id}`, {
		headers: {
	...
	if(accent_color) {
		context.strokeStyle = this.rgbToHex(accent_color);
		context.fillStyle = this.rgbToHex(accent_color);
	} else {
